...
export class ToolRegistry {
  ...
  constructor(ghlClient: GHLApiClient, locationId?: string) {
    this.initModules(ghlClient, locationId);
  }

  private initModules(ghl: GHLApiClient, locationId?: string): void {
    ...
    // Workflow Builder — internal API with Firebase auth; pass locationId from request
    // so per-request sub-account switching (x-ghl-location-id header) works correctly.
    const workflowBuilderTools = new WorkflowBuilderTools(locationId);
